...
type NavView = "Dashboard" | "Patients" | "Lab Report Entry" | "Verification" | "Team" | "Activity" | "History" | "Settings";
...
function AuthScreen({ onAuthenticated, initialError = "" }: { onAuthenticated: (session: { user: PlatformUser; profile: StaffProfile }) => void; initialError?: string }) {
  ...
  return <main className="auth-shell">
    <section className="auth-story" aria-label="RapidLab introduction">
      <Logo />
      <div className="auth-story-copy"><p className="eyebrow"><Zap /> Laboratory record workflow</p><h1>Move from <em>paper</em> to <em>patient</em> in seconds.</h1><p>Fast laboratory record entry for hospitals, clinics and diagnostic centres.</p></div>
      ...
      <Tabs value={mode} onValueChange={(value) => changeMode(value as "signin" | "signup")}>
        ...
        <form onSubmit={submit}>
          <TabsContent value="signin" className="auth-form">
            <Field label="Email address" value={email} onChange={setEmail} placeholder="staff@hospital.org" type="email" />
          ...
          <TabsContent value="signup" className="auth-form">
            {signupStep === "details" ? <>
              <div className="auth-field-row"><Field label="Full name" value={name} onChange={setName} placeholder="Staff member name" /><Field label="Staff ID" value={staffId} onChange={setStaffId} placeholder="Example: STF-204" /></div>
              <Field label="Email address" value={email} onChange={setEmail} placeholder="staff@hospital.org" type="email" />
...
function Dashboard({ profile, onProfileChange, onLogout }: { profile: ActiveProfile; onProfileChange: (profile: StaffProfile) => void; onLogout: () => void }) {
  ...
  const navItems: { label: NavView; icon: typeof LayoutDashboard }[] = [
    ...
    ...(role === "nurse" ? [{ label: "Lab Report Entry" as NavView, icon: Zap }] : []),
  ...
  return <SidebarProvider>
    ...
    <SidebarInset className="dashboard-shell">
      ...
      <main className="dashboard-main">
        ...
        {view === "Lab Report Entry" && <LabEntryWorkspace records={records} onManual={() => startEntry("manual")} onScan={() => startEntry("scan")} onView={setSelectedRecord} />}
...
function DashboardOverview({ staffName, hospitalName, dateLabel, records, onManual, onScan, onView }: { staffName: string; hospitalName: string; dateLabel: string; records: PatientRecord[]; onManual: () => void; onScan: () => void; onView: (record: PatientRecord) => void }) {
  return <div className="workspace-page dashboard-view">
    <section className="welcome-row"><div><p className="eyebrow">{dateLabel}</p><h1>Good day, {staffName.split(" ")[0]}</h1><p>Nurse · {hospitalName} shared laboratory workspace</p></div><Button className="new-patient" onClick={onManual}><Plus /> New patient record</Button></section>
    <section className="metrics" aria-label="Daily summary">
      <Metric icon={UsersRound} label="Patient Records" value={String(records.length)} note="Hospital-wide records" tone="green" />
...
function LabEntryWorkspace({ records, onManual, onScan, onView }: { records: PatientRecord[]; onManual: () => void; onScan: () => void; onView: (record: PatientRecord) => void }) {
  return <div className="workspace-page lab-entry-view">
    <PageHeading eyebrow="Laboratory records" title="Lab Report Entry" description="Choose how you want to enter the patient’s laboratory report."><span className="live-badge"><i /> Ready for entry</span></PageHeading>
