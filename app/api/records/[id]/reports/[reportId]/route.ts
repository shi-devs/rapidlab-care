...
import { deleteReport, readReport } from "@/lib/report-storage";
...
export async function GET(_request: Request, context: { params: Promise<{ id: string; reportId: string }> }) {
  ...
  const object = await readReport(report.fileKey);
  ...
  return new Response(object.body, {
    headers: {
      "Content-Type": object.contentType || report.contentType || "application/octet-stream",
  ...
  try {
    ...
    if (reportId === "legacy") {
      ...
      await db.transaction(async (tx) => {
        await tx.update(labRecords).set({ reportFileKey: null, reportFileName: null, status: "pending", verifiedByEmail: null, verifiedAt: null, updatedAt: now })
          .where(and(eq(labRecords.id, id), eq(labRecords.hospitalId, hospitalId)));
        await tx.insert(recordAudit).values({
      ...
      await db.transaction(async (tx) => {
        await tx.delete(labReportFiles).where(and(eq(labReportFiles.id, reportId), eq(labReportFiles.recordId, id), eq(labReportFiles.hospitalId, hospitalId)));
        await tx.update(labRecords).set({ status: "pending", verifiedByEmail: null, verifiedAt: null, updatedAt: now })
          .where(and(eq(labRecords.id, id), eq(labRecords.hospitalId, hospitalId)));
        await tx.insert(recordAudit).values({
          id: crypto.randomUUID(), recordId: id, hospitalId,
          actorEmail: staff.user.email, actorName: staff.profile.name,
    ...
    try { await deleteReport(fileKey); } catch { cleanupPending = true; }
