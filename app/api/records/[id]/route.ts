...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      await tx.delete(labReportFiles).where(and(eq(labReportFiles.recordId, id), eq(labReportFiles.hospitalId, hospitalId)));
      await tx.delete(labRecords).where(and(eq(labRecords.id, id), eq(labRecords.hospitalId, hospitalId)));
    ...
    const cleanup = await Promise.allSettled(fileKeys.map((key) => deleteReport(key)));
