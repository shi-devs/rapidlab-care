  ...
  try {
    ...
    if (profile[0]) {
      await db.transaction(async (tx) => {
        await tx.insert(authAccounts).values(accountValues);
        await tx.delete(emailVerificationChallenges).where(eq(emailVerificationChallenges.email, email));
      });
    } else {
      await db.transaction(async (tx) => {
        await tx.insert(staffProfiles).values({ email, name: challenge.name, staffId: challenge.staffId, createdAt: now, updatedAt: now });
        await tx.insert(authAccounts).values(accountValues);
        await tx.delete(emailVerificationChallenges).where(eq(emailVerificationChallenges.email, email));
      });
    }
