...
export async function DELETE() {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      await tx.delete(authSessions).where(eq(authSessions.email, email));
      await tx.delete(authAccounts).where(eq(authAccounts.email, email));
      await tx.delete(emailVerificationChallenges).where(eq(emailVerificationChallenges.email, email));
      await tx.delete(hospitalMembers).where(eq(hospitalMembers.email, email));
      await tx.delete(staffProfiles).where(eq(staffProfiles.email, email));
    });
    await clearSession();
